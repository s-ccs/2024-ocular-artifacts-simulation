...
=== Scalp Topography
#todo("explain topography", inline: true)

=== Source Dipole and Lead Field
In general, electric potential differences present in human tissues can be represented by means of electrical current dipoles placed at different points in the space occupied by the tissues @malmivuo_bioelectromagnetismprinciples_1995. When a current dipole is placed in a volume conductor, it creates an electrical potential field around it. The resulting potential at any point in space can be described by a set of three vectors in the three coordinate directions, and this is known as the "lead vector" or "lead field". For a specific orientation of the source dipole, the resultant potential can be calculated by multiplying the lead vector by the source dipole's orientation.

#figure(
  image("../src/graphics/fig source dipole.png", width: 30%),
  caption: [Current dipole representing a potential difference.],
) <source_dipole>
#todo("1-line explanation volume conductor", inline: true)

=== EEG Head Model <eeg-head-model>
#todo("explain head model from m/eeg primer ch3 pg 35 & forward model specifically", inline: true)
In EEG research, a 'head model' describes the head and the biological tissues in it as a conducting medium that conducts electrical potentials. The simplest head model is a homogeneous sphere made out of the same material throughout. In reality, different tissues have different conductivities, and the head model can be improved by modifying this basic model to contain multiple 3d sections representing the different tissues in the head, and assigning each section the conductivity of its respective tissue type. 

Using the concept of a head model and that of using source dipoles to represent electrical activity in the brain, muscles, or eyes, we can build a model consisting of a set of source dipoles placed at different locations in the head. Using these, we can further calculate the resulting potentials due to each of these measured at different locations on the head surface (i.e. the scalp), and combine the two - this is known as a forward model @malmivuo_bioelectromagnetismprinciples_1995. 

In this project, we will use the #abbr.a("HArtMuT") head model as a forward model to simulate scalp potentials.

=== Electro-Oculogram (EOG)
#todo("explain EOG? check this at the end, is EOG being used often enough to warrant a separate section for it?", inline: true)
...
#todo("figure: EEG plot showing artefacts. e.g. Primer fig. 8.4 shows eye charges & dipole, and the book also has other diagrams showing different artefacts.", inline:true)
...
We will now discuss in some more detail the origin of artefacts specifically due to eyeball movement and some methods of modelling the eyeballs in order to later simulate these artefacts.


== Eye structure and corneo-retinal dipole (CRD)

The eye is composed of various types of tissue (e.g. cornea, retina, sclera, choroid, and so on), and certain sections of the eyeball are also filled with fluid (aqueous and vitreous humor). Each of these tissues and fluids have their own electrical properties, in particular their electrical conductivity, which influences how they conduct electrical potential. Certain structures also have inherent charge distributions caused due to properties of the cells making up the tissue. 
...
#todo("add some info about sliding electrode effect (but mention that it is not directly relevant to this project right now)", inline: true)

Although sources agree that the retina is positive on the inside of the eyeball compared to the outside, there is some difference of opinion about the contribution of the cornea to the corneo-retinal dipole. According to #pc[@lins_ocular_1993-1], the outer side of the cornea is negative relative to the inner side. #pc[@berg_dipole_1991] and #pc[@plochl_combining_2012] in their discussions of the corneo-retinal dipole do not mention the charge of the cornea itself at all. However, the concept of the corneo-retinal dipole as it is described here (i.e., a potential difference between the front and back of the eye, being positive at the front compared to the back) is common to all the previous studies reviewed in this area.
...
#todo("mention electrode layout system used in hartmut", inline:true)
...
We carried out two kinds of simulations: First, using our model of multiple retinal and corneal source dipoles; and second, simulating the CRD as a single dipole at the center of each eye (resultant dipole method). 
...
The results of the biological and CRD simulations agree with the conceptual idea - the difference topographies are positive on the left side and negative to the right when the cornea moves from the center to the left and the retina accordingly from the center to the right. This is also similar to the results from @plochl_combining_2012. 
...
Further, although we have assumed a simplified scenario with both eyes looking in the same direction (implying a stimulus located far away from the subject), the stimuli in EEG studies are presented on a screen in front of the subject, which means that the eye gaze directions are not perfectly parallel. 
...
In this project, we presented a model of the human eye represented by a set of electrical current dipoles ("source dipoles") placed at various locations in the eye. We implemented two methods to simulate the scalp topography for any given gaze direction of the eye: the "ensemble" method, biologically-motivated and using many electrical source dipoles on the surface of each eye, and the "corneo-retinal dipole" method using just one source dipole per eye, placed in the eye center. 

Next, as a proof of concept, we simulated the scalp potentials ("lead fields") at the start and at the end of an eye movement. After calculating the difference between these two lead fields, we plotted the scalp topography of this difference in order to compare the two methods. The eye movements considered were a pure horizontal and a pure vertical eye movement, defined by the start and end position of the eye.
